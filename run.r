...
#LDA
library('MASS')
CV_CV<-trainDF_subset[1:100000,] #use remaining testdata as validation set for CV
trainDF_CV<-trainDF_subset[100001:878049,] #try first 50000 entries as training set


lda.fit =lda(Category ~ timeDiff + X+Y , data=trainDF_CV)

testDF_subset<-testDF_subset[,names(testDF_subset)[-c(1)]] 
#get rid of DayOfWeek since LDA only handles non-categorical predictors
...
formula = as.formula(Category ~ timeDiff + X+Y)
